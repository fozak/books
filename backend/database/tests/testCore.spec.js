...
import { assertDoesNotThrow, assertThrows, getBuiltTestSchemaMap, } from './helpers';
/**
...
async function getDb(shouldMigrate = true) {
...
test('Post Migrate TableInfo', async function (t) {
    ...
    for (const schemaName in schemaMap) {
        ...
        t.equal(columns.length, columnCount, `${schemaName}:: column count: ${columns.length}, ${columnCount}`);
        for (const column of columns) {
            const field = fieldMap[column.name];
            const dbColType = sqliteTypeMap[field.fieldtype];
            t.equal(column.name, field.fieldname, `${schemaName}.${column.name}:: name check: ${column.name}, ${field.fieldname}`);
            t.equal(column.type.toLowerCase(), dbColType, `${schemaName}.${column.name}:: type check: ${column.type}, ${dbColType}`);
            if (field.required !== undefined) {
                t.equal(!!column.notnull, field.required, `${schemaName}.${column.name}:: iotnull iheck: ${column.notnull}, ${field.required}`);
            }
            else {
                t.equal(column.notnull, 0, `${schemaName}.${column.name}:: notnull check: ${column.notnull}, ${field.required}`);
            }
            if (column.dflt_value === null) {
                t.equal(field.default, undefined, `${schemaName}.${column.name}:: dflt_value check: ${column.dflt_value}, ${field.default}`);
            }
            else {
                t.equal(column.dflt_value.slice(1, -1), String(field.default), `${schemaName}.${column.name}:: dflt_value check: ${column.type}, ${dbColType}`);
            }
        }
    }
    await db.close();
...
test('CRUD single values', async function (t) {
     ...
     * takes place.
     */
    let rows = await db.knex.raw('select * from SingleValue');
    const defaultMap = getValueMapFromList(schemaMap.SystemSettings.fields, 'fieldname', 'default');
    for (const row of rows) {
        t.equal(row.value, defaultMap[row.fieldname], `${row.fieldname} default values equality`);
    }
    /**
    ...
    const localeEntryName = localeRow?.name;
    const localeEntryCreated = localeRow?.created;
    let locale = 'hi-IN';
    ...
    t.equal(localeRow?.name, localeEntryName, `localeEntryName ${localeRow?.name}, ${localeEntryName}`);
    t.equal(localeRow?.value, locale, `locale ${localeRow?.value}, ${locale}`);
    t.equal(localeRow?.created, localeEntryCreated, `locale ${localeRow?.value}, ${locale}`);
    /**
     * Update
     */
    locale = 'ca-ES';
    ...
    for (const sv of svl) {
        ...
        t.equal(sv.value, { locale, dateFormat }[sv.fieldname], `singleValue value ${sv.value}`);
        /**
         * get
         */
...
test('CRUD nondependent schema', async function (t) {
    ...
    await assertThrows(async () => await db.insert(schemaName, { name }), 'insert() did not throw without meta values');
    ...
    for (const key in metaValues) {
        t.equal(firstRow[key], metaValues[key], `${key} check`);
    }
    /**
     * Update
     */
    ...
    t.equal(Object.keys(fvMap).length, 0, `key count get ${JSON.stringify(fvMap)}`);
    /**
     * > 1 entries
     */
    ...
    for (const i in cs) {
        for (const k in cs[i]) {
            const val = cs[i][k];
            t.equal(rows?.[i]?.[k], val, `equality check ${i} ${k} ${val} ${rows?.[i]?.[k]}`);
        }
    }
    // Update
    ...
    t.equal(Object.keys(fvMap).length, 0, `mi rename check old ${JSON.stringify(fvMap)}`);
...
test('CRUD dependent schema', async function (t) {
    ...
    await assertThrows(async () => await db.insert(SalesInvoice, invoice), 'foreign key constraint fail failed');
    await assertDoesNotThrow(async () => {
        await db.insert(Customer, customer);
        await db.insert(SalesInvoice, invoice);
    }, 'insertion failed');
    await assertThrows(async () => await db.delete(Customer, customer.name), 'foreign key constraint fail failed');
    await assertDoesNotThrow(async () => {
        await db.delete(SalesInvoice, invoice.name);
        await db.delete(Customer, customer.name);
    }, 'deletion failed');
    await db.insert(Customer, customer);
    await db.insert(SalesInvoice, invoice);
    let fvMap = await db.get(SalesInvoice, invoice.name);
    for (const key in invoice) {
        let expected = invoice[key];
        if (typeof expected === 'boolean') {
            expected = +expected;
        }
        t.equal(fvMap[key], expected, `equality check ${key}: ${fvMap[key]}, ${invoice[key]}`);
    }
    t.equal(fvMap.items?.length, 0, 'empty items check');
    const items = [
        {
            item: 'Bottle Caps',
            quantity: 2,
    ...
    await assertThrows(async () => await db.insert(SalesInvoice, { name: invoice.name, items }), 'invoice insertion with ct did not fail');
    await assertDoesNotThrow(async () => await db.update(SalesInvoice, { name: invoice.name, items }), 'ct insertion failed');
    fvMap = await db.get(SalesInvoice, invoice.name);
    const ct = fvMap.items;
    t.equal(ct.length, 1, `ct length ${ct.length}`);
    t.equal(ct[0].parent, invoice.name, `ct parent ${ct[0].parent}`);
    t.equal(ct[0].parentFieldname, 'items', `ct parentFieldname ${ct[0].parentFieldname}`);
    t.equal(ct[0].parentSchemaName, SalesInvoice, `ct parentSchemaName ${ct[0].parentSchemaName}`);
    for (const key in items[0]) {
        t.equal(ct[0][key], items[0][key], `ct values ${key}: ${ct[0][key]}, ${items[0][key]}`);
    }
    items.push({
        item: 'Mentats',
        quantity: 4,
    ...
    await assertDoesNotThrow(async () => await db.update(SalesInvoice, { name: invoice.name, items }), 'ct updation failed');
    ...
    for (const i in rows) {
        for (const key in rows[i]) {
            t.equal(rows[i][key], items[i][key], `ct values ${i},${key}: ${rows[i][key]}`);
        }
    }
...
test('db deleteAll', async (t) => {
    ...
    // Single filter
    t.equal(await db.deleteAll('Customer', { email: emailOne }), customers.filter((c) => c.email === emailOne).length);
    t.equal((await db.getAll('Customer', { filters: { email: emailOne } })).length, 0);
    // Multiple filters
    t.equal(await db.deleteAll('Customer', { email: emailTwo, phone: phoneTwo }), customers.filter(({ phone, email }) => email === emailTwo && phone === phoneTwo).length);
    t.equal(await db.deleteAll('Customer', { email: emailTwo, phone: phoneTwo }), 0);
    // Includes filters
    t.equal(await db.deleteAll('Customer', { email: ['in', [emailTwo, emailThree]] }), customers.filter(({ email, phone }) => [emailTwo, emailThree].includes(email) &&
        !(phone === phoneTwo && email === emailTwo)).length);
    t.equal((await db.getAll('Customer', {
        filters: { email: ['in', [emailTwo, emailThree]] },
    })).length, 0);
    await db.close();
});
//# sourceMappingURL=testCore.spec.js.map