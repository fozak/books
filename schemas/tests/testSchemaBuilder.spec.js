...
import { addMetaFields, cleanSchemas, getAbstractCombinedSchemas, getRegionalCombinedSchemas, setSchemaNameOnFields, } from '../index';
...
test('Schema Equality with App Schemas', function (t) {
    ...
    t.equal(isEqual(regionalCombined.JournalEntry, appSchemaMap.JournalEntry), true);
    t.equal(isEqual(regionalCombined.JournalEntryAccount, appSchemaMap.JournalEntryAccount), true);
...
test('Schema Equality with App Schemas', function (t) {
    ...
    t.equal(isEqual(abstractCombined.JournalEntry, appSchemaMap.JournalEntry), true);
    t.equal(isEqual(abstractCombined.JournalEntryAccount, appSchemaMap.JournalEntryAccount), true);
    t.equal(isEqual(abstractCombined.Customer, appSchemaMap.Customer), false);
    t.end();
});
test('Schema Field Existance', function (t) {
    t.equal(everyFieldExists(regionalSchemaMap.Party.quickEditFields ?? [], abstractCombined.Customer), true);
    t.end();
});
...
test('Schema Field Existance', function (t) {
    ...
    t.equal(someFieldExists(subtract(allFieldNames, baseFieldNames), finalSchemas.Customer), false);
    t.equal(everyFieldExists([...baseFieldNames, ...submittableFieldNames], finalSchemas.JournalEntry), true);
    t.equal(someFieldExists(subtract(allFieldNames, baseFieldNames, submittableFieldNames), finalSchemas.JournalEntry), false);
    t.equal(everyFieldExists(childFieldNames, finalSchemas.JournalEntryAccount), true);
    t.equal(someFieldExists(subtract(allFieldNames, childFieldNames), finalSchemas.JournalEntryAccount), false);
    t.equal(everyFieldExists([...treeFieldNames, ...baseFieldNames], finalSchemas.Account), true);
    t.equal(someFieldExists(subtract(allFieldNames, treeFieldNames, baseFieldNames), finalSchemas.Account), false);
    t.end();
});
//# sourceMappingURL=testSchemaBuilder.spec.js.map