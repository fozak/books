    ...
    constructor() {
        ...
        this.validations = {
            label: (value) => {
                ...
                this.validations.fieldname(fieldname);
            },
            fieldname: (value) => {
                if (typeof value !== 'string') {
                    return;
                ...
                if (field && !field.isCustom) {
                    throw new ValueError(this.fyo.t `Fieldname ${value} already exists for ${this.parentdoc
                        .name}`);
                }
                const cf = this.parentdoc?.customFields?.find((cf) => cf.fieldname === value);
                if (cf) {
                    throw new ValueError(this.fyo.t `Fieldname ${value} already used for Custom Field ${(cf.idx ?? 0) + 1}`);
                }
            },
        };
        this.required = {
