...
export function areDocValuesEqual(dvOne, dvTwo) {
    if (['string', 'number'].includes(typeof dvOne) || dvOne instanceof Date) {
        return dvOne === dvTwo;
    }
    if (isPesa(dvOne)) {
        try {
            return dvOne.eq(dvTwo);
        }
        catch {
            return false;
        }
...
export function getPreDefaultValues(fieldtype, fyo) {
    switch (fieldtype) {
...
export function getMissingMandatoryMessage(doc) {
    ...
    const message = mandatoryFields
        ...
        if (f.fieldtype === FieldTypeEnum.Table) {
            return isNullOrUndef || value?.length === 0;
        }
...
export function shouldApplyFormula(field, doc, fieldname) {
...
function shouldApplyFormulaPreSync(fieldname, dependsOn, doc) {
...
export function isDocValueTruthy(docValue) {
    if (isPesa(docValue)) {
...
export function setChildDocIdx(childDocs) {
    childDocs.forEach((cd, idx) => {
        cd.idx = idx;
    });
}
export function getFormulaSequence(formulas) {
...
function sequenceDependencies(depMap) {
    /**
