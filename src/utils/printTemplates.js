...
export async function getPrintTemplatePropValues(doc) {
    const fyo = doc.fyo;
    let paymentId;
    let sinvDoc;
    const values = { doc: {}, print: {} };
    values.doc = await getPrintTemplateDocValues(doc);
    if (values.doc.entryType === ModelNameEnum.SalesInvoice ||
        values.doc.entryType === ModelNameEnum.PurchaseInvoice) {
        paymentId = await doc.getPaymentIds();
        if (paymentId && paymentId.length) {
            const paymentDetails = await getPaymentDetails(doc, paymentId);
            values.doc.paymentDetails = paymentDetails;
        }
    }
    if (doc.referenceType == ModelNameEnum.SalesInvoice) {
        const referenceName = doc?.for[0]?.referenceName;
        if (referenceName) {
            sinvDoc = await fyo.doc.getDoc(ModelNameEnum.SalesInvoice, referenceName);
            if (sinvDoc.taxes) {
                values.doc.taxes = sinvDoc.taxes;
            }
        }
    }
    let totalTax;
    if (values.doc.entryType !== ModelNameEnum.Shipment) {
        totalTax = await (sinvDoc ?? doc)?.getTotalTax();
    }
    if (doc.schema.name == ModelNameEnum.Payment) {
        values.doc.amountPaidInWords = getGrandTotalInWords(doc.amountPaid?.float);
    }
    values.doc.subTotal = doc.fyo.format((doc.grandTotal ?? doc.amount).sub(totalTax || 0), ModelNameEnum.Currency);
    ...
    const accountingValues = await getPrintTemplateDocValues(accountingSettings, accountingSettingsFields);
    values.print = {
        ...printValues,
        ...accountingValues,
    };
    ...
    if (discountSchema.some((value) => doc.schemaName?.endsWith(value))) {
        values.doc.totalDiscount =
            formattedTotalDiscount(doc);
    }
    values.doc.showHSN = showHSN(doc);
    values.doc.grandTotalInWords = getGrandTotalInWords((doc.grandTotal ?? doc.amount).float);
    values.doc.date = getDate(doc.date);
    if (printSettings.displayTime) {
        values.doc.time = getTime(doc.date);
    }
    if (printSettings.displayDescription) {
        values.doc.description = showDescription(doc);
    }
    return values;
}
async function getPaymentDetails(doc, paymentId) {
    const paymentIds = paymentId.sort();
    const paymentDetails = [];
    let outstandingAmount = doc.grandTotal;
    for (const payment of paymentIds) {
        const paymentDoc = await doc.fyo.doc.getDoc(ModelNameEnum.Payment, payment);
        outstandingAmount = outstandingAmount.sub(paymentDoc.amount);
        paymentDetails.push({
...
export function getPrintTemplatePropHints(schemaName, fyo) {
    const hints = {};
    const schema = fyo.schemaMap[schemaName];
    hints.doc = getPrintTemplateDocHints(schema, fyo);
    const printSettingsHints = getPrintTemplateDocHints(fyo.schemaMap[ModelNameEnum.PrintSettings], fyo, printSettingsFields);
    const accountingSettingsHints = getPrintTemplateDocHints(fyo.schemaMap[ModelNameEnum.AccountingSettings], fyo, accountingSettingsFields);
    hints.print = {
        ...printSettingsHints,
        ...accountingSettingsHints,
    };
    if (schemaName?.endsWith('Invoice')) {
        hints.doc.totalDiscount = fyo.t `Total Discount`;
        hints.doc.showHSN = fyo.t `Show HSN`;
    }
    return hints;
}
function getGrandTotalInWords(total) {
...
function showHSN(doc) {
    const items = doc.items;
    if (!Array.isArray(items)) {
        return false;
    }
    return items.map((i) => i.hsnCode).every(Boolean);
}
function showDescription(doc) {
    const description = Array.isArray(doc.items)
        ? doc.items.map((item) => item.description).filter(Boolean)
        : [];
    return description.length > 0;
}
function formattedTotalDiscount(doc) {
...
function getPrintTemplateDocHints(schema, fyo, fieldnames, linkLevel) {
    linkLevel ?? (linkLevel = 0);
    const hints = {};
    const links = {};
...
async function getPrintTemplateDocValues(doc, fieldnames) {
    const values = {};
...
export async function getPathAndMakePDF(name, innerHTML, width, height, shouldPrint) {
    if (!shouldPrint) {
...
function constructPrintDocument(innerHTML) {
...
export async function updatePrintTemplates(fyo) {
...
function getPrintTemplateUpdateList({ file, template, modified: modifiedString, width, height }, nameModifiedMap, fyo) {
    const templateList = [];
...
function getNameAndTypeFromTemplateFile(file, fyo) {
    /**
...
`;
//# sourceMappingURL=printTemplates.js.map