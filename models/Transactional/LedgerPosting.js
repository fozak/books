...
export class LedgerPosting {
    ...
    timezoneDateTimeAdjuster(setDate) {
    ...
    _getLedgerEntry(account, type) {
        ...
        const ledgerEntry = this.fyo.doc.getNewDoc(ModelNameEnum.AccountingLedgerEntry, {
            account: account,
            party: this.refDoc.party ?? '',
            date: this.timezoneDateTimeAdjuster(this.refDoc.date),
    ...
    _validateIsEqual() {
        ...
        throw new ValidationError(t `Total Debit: ${this.fyo.format(debit, 'Currency')} must be equal to Total Credit: ${this.fyo.format(credit, 'Currency')}`);
    }
    ...
    async _syncReverseLedgerEntries() {
        ...
        for (const { name } of data) {
            const doc = (await this.fyo.doc.getDoc('AccountingLedgerEntry', name));
            await doc.revert();
        }
    }
    async _getRoundOffAccount() {
        const roundOffAccount = (await this.fyo.getValue(ModelNameEnum.AccountingSettings, 'roundOffAccount'));
        if (!roundOffAccount) {
            const notFoundError = new NotFoundError(t `Please set Round Off Account in the Settings.`, false);
...
//# sourceMappingURL=LedgerPosting.js.map