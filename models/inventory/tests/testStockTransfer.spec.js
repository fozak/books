...
test('insert test docs', async (t) => {
    ...
    t.ok(await fyo.db.exists(ModelNameEnum.Location, location), 'location created');
    t.ok(await fyo.db.exists(ModelNameEnum.Item, item), 'item created');
});
test('inventory settings', async (t) => {
    const doc = (await fyo.doc.getDoc(ModelNameEnum.InventorySettings));
...
test('PurchaseReceipt, create inward stock movement', async (t) => {
    ...
    const rate = testDocs['Item'][item].rate ?? 0;
    const quantity = 10;
    const doc = await getStockTransfer(ModelNameEnum.PurchaseReceipt, party, date, [
        {
            item,
            location,
            quantity,
    ...
    await doc.submit();
    t.equal((await fyo.db.getAllRaw(ModelNameEnum.PurchaseReceipt)).length, 1, 'purchase receipt created');
    t.equal((await getSLEs(doc.name, doc.schemaName, fyo)).length, 1, 'sle created');
    t.equal(await fyo.db.getStockQuantity(item, location), quantity, 'stock purchased');
...
test('Shipment, create outward stock movement', async (t) => {
    ...
    const rate = testDocs['Item'][item].rate ?? 0;
    const quantity = 5;
    const doc = await getStockTransfer(ModelNameEnum.Shipment, party, date, [
        {
            item,
            location,
            quantity,
    ...
    await doc.submit();
    t.equal((await fyo.db.getAllRaw(ModelNameEnum.Shipment)).length, 1, 'shipment created');
    t.equal((await getSLEs(doc.name, doc.schemaName, fyo)).length, 1, 'sle created');
    t.equal(await fyo.db.getStockQuantity(item, location), 10 - quantity, 'stock purchased');
...
test('Shipment, invalid', async (t) => {
    ...
    t.equal((await getSLEs(doc.name, doc.schemaName, fyo)).length, 0, 'sles not created');
    t.equal((await getALEs(doc.name, doc.schemaName, fyo)).length, 0, 'ales not created');
});
test('Stock Transfer, invalid cancellation', async (t) => {
    const { name } = (await fyo.db.getAllRaw(ModelNameEnum.PurchaseReceipt))[0] ?? {};
    ...
    t.equal(await fyo.db.getStockQuantity(item, location), 5, 'stock unchanged');
    t.equal((await getSLEs(name, doc.schemaName, fyo)).length, 1, 'sle unchanged');
...
test('Shipment, cancel and delete', async (t) => {
    const { name } = (await fyo.db.getAllRaw(ModelNameEnum.Shipment, { order: 'asc' }))[0] ?? {};
    ...
    t.equal((await fyo.db.getAllRaw(ModelNameEnum.Shipment, {
        filters: { name: name },
    })).length, 0, 'doc deleted');
});
test('Purchase Receipt, cancel and delete', async (t) => {
    const { name } = (await fyo.db.getAllRaw(ModelNameEnum.PurchaseReceipt, {
        order: 'asc',
    }))[0] ?? {};
...
test('Purchase Invoice then Purchase Receipt', async (t) => {
    const rate = testDocs.Item[item].rate;
    const quantity = 3;
    ...
    t.equal(ModelNameEnum.PurchaseReceipt, prec.schemaName, 'stock transfer is a PREC');
...
test('Back Ref Purchase Receipt cancel', async (t) => {
    const prec = (await fyo.doc.getDoc(ModelNameEnum.PurchaseReceipt, 'PREC-1002'));
    t.equal(prec.backReference, 'PINV-1001', 'back reference matches');
    await assertDoesNotThrow(async () => {
        await prec.cancel();
    });
    const pinv = (await fyo.doc.getDoc(ModelNameEnum.PurchaseInvoice, 'PINV-1001'));
    t.equal(pinv.stockNotTransferred, 3, 'pinv stock untransferred');
    t.equal(pinv.items?.[0].stockNotTransferred, 3, 'pinv item stock untransferred');
});
...
test('Sales Invoice then partial Shipment', async (t) => {
    ...
    t.equal(sinv.items?.[0].stockNotTransferred, 2, 'item stock qty 2 has not been transferred');
});
test('Sales Invoice then another Shipment', async (t) => {
    const sinv = (await fyo.doc.getDoc(ModelNameEnum.SalesInvoice, 'SINV-1001'));
    const shpm = await sinv.getStockTransfer();
    if (shpm === null) {
        return t.ok(false, 'shpm was null');
    }
    ...
    t.equal(sinv.items?.[0].stockNotTransferred, 0, 'item stock has been transferred');
...
test('Cancel Sales Invoice after Shipment is created', async (t) => {
    const sinv = (await fyo.doc.getDoc(ModelNameEnum.SalesInvoice, 'SINV-1001'));
    await assertThrows(async () => await sinv.cancel(), 'cancel prevent cause of SHPM');
    const ales = await fyo.db.getAllRaw(ModelNameEnum.AccountingLedgerEntry, {
        fields: ['name', 'reverted'],
...
test('Cancel partial Shipment', async (t) => {
    let shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, 'SHPM-1003'));
    ...
    shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, 'SHPM-1004'));
...
test('Duplicate Shipment, backref unset', async (t) => {
    const shpm = (await fyo.doc.getDoc(ModelNameEnum.Shipment, 'SHPM-1003'));
...
test('Cancel and Delete Sales Invoice with cancelled Shipments', async (t) => {
    ...
    t.notOk(await fyo.db.exists(ModelNameEnum.SalesInvoice, 'SINV-1001'), 'SINV-1001 deleted');
    for (const { name } of transfers) {
        t.notOk(await fyo.db.exists(ModelNameEnum.Shipment, 'SINV-1001'), `linked Shipment ${name} deleted`);
    }
});
test('Create Shipment from manually set Back Ref', async (t) => {
    const rate = testDocs['Item'][item].rate ?? 0;
    const totalQuantity = 10;
    const prec = await getStockTransfer(ModelNameEnum.PurchaseReceipt, party, new Date('2022-01-08'), [
        {
            item,
            location,
    ...
    const shpm = fyo.doc.getNewDoc(ModelNameEnum.Shipment);
    ...
    t.equal(await fyo.db.getStockQuantity(item, location), totalQuantity - quantity, 'quantity shipped');
    t.equal(sinv.stockNotTransferred, 0, 'stock has been transferred');
});
test('Create Shipment then create return against it', async (t) => {
    ...
    const shpmReturn = (await shpm.getReturnDoc());
    await shpmReturn.sync();
    await shpmReturn.submit();
    t.equal(shpmReturn.name, 'SHPM-1007', 'Shipment return created');
    t.ok(shpmReturn.grandTotal?.isNegative(), 'Shipment return doc has negative grand total ');
    ...
    for (const ale of returnShpmAles) {
        if (ale.account === 'Stock In Hand') {
            t.equal(fyo.pesa(ale.debit).float, shpmReturn.grandTotal?.float, `Shipment return amount debited from ${ale.account}`);
        }
        if (ale.account === 'Cost of Goods Sold') {
            t.equal(fyo.pesa(ale.credit).float, shpmReturn.grandTotal?.float, `Shipment return amount credited to ${ale.account}`);
        }
    }
});
test('Create Shipment return of batched item', async (t) => {
    ...
    newItemDoc.sync();
    newItemDoc.submit();
    t.ok(fyo.db.exists(ModelNameEnum.Item, item.name), `item ${item.name} created`);
    const batches = [
        {
            name: 'JKT-A-2',
    ...
    for (const batch of batches) {
        ...
        batchDoc.sync();
        t.ok(fyo.db.exists(ModelNameEnum.Batch, batch.name), `Batch ${batch.name} created`);
    }
    ...
    const shipmentDoc = fyo.doc.getNewDoc(ModelNameEnum.Shipment);
    ...
    const shpmReturnDoc = fyo.doc.getNewDoc(ModelNameEnum.Shipment);
    ...
    const secondReturnDoc = (await shipmentDoc.getReturnDoc());
    for (const item of secondReturnDoc.items) {
        if (item.batch == batches[0].name) {
            const docItemQty = shipmentDoc.items[0].quantity;
            const retItemQty = shpmReturnDoc.items[0].quantity;
            const balanceQty = retItemQty - docItemQty;
            t.equal(item.quantity, balanceQty, `Batch ${item.batch} has ${balanceQty} qty left to return`);
        }
        if (item.batch == batches[1].name) {
            const docItemQty = shipmentDoc.items[1].quantity;
            const retItemQty = shpmReturnDoc.items[1].quantity;
            const balanceQty = retItemQty - docItemQty;
            t.equal(item.quantity, balanceQty, `Batch ${item.batch} has ${balanceQty} qty left to return`);
        }
    }
...
test('Create Purchase Reciept then create return against it', async (t) => {
    const rate = testDocs.Item[item].rate;
    const prec = fyo.doc.getNewDoc(ModelNameEnum.PurchaseReceipt);
    await prec.set({
        party,
        date: new Date('2023-05-18'),
        items: [{ item, quantity: 3, rate }],
    ...
    const precReturn = (await prec.getReturnDoc());
    await precReturn.sync();
    await precReturn.submit();
    t.equal(precReturn.name, 'PREC-1006', 'Purchase Receipt return created');
    t.ok(precReturn.grandTotal?.isNegative(), 'Purchase Receipt return doc has negative grand total ');
    ...
    for (const ale of returnPrecAles) {
        if (ale.account === 'Stock In Hand') {
            t.equal(fyo.pesa(ale.credit).float, precReturn.grandTotal?.float, `Purchase Receipt return amount credited to ${ale.account}`);
        }
        if (ale.account === 'Stock Received But Not Billed') {
            t.equal(fyo.pesa(ale.debit).float, precReturn.grandTotal?.float, `Purchase Receipt return amount debited from ${ale.account}`);
        }
    }
});
test('Create Purchase Reciept return of serialized item', async (t) => {
    ...
    t.equal(precDoc.name, 'PREC-1007', 'purchase reciept having serial number created');
    ...
    t.equal(returnPrecDoc.name, 'PREC-1008', 'purchase reciept return having serial number created');
    const secondPrecReturnDoc = (await precDoc.getReturnDoc());
    ...
    for (const serialNumber of returnedSerialNumbers) {
        ...
        t.equal(status, 'Delivered', 'serial number status updated to Delivered after return');
    }
    for (const serialNumber of returnBalSerialNumbers) {
        const status = await fyo.getValue(ModelNameEnum.SerialNumber, serialNumber, 'status');
        t.equal(status, 'Active', 'serial number status is Active for unreturned item');
    }
});
closeTestFyo(fyo, __filename);
//# sourceMappingURL=testStockTransfer.spec.js.map