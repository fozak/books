...
test('create stock movement, material transfer', async (t) => {
    ...
    t.equal(await fyo.db.getStockQuantity(itemMap.Ink.name, locationMap.LocationOne), 0);
    t.equal(await fyo.db.getStockQuantity(itemMap.Ink.name), quantity);
});
test('create stock movement, material issue', async (t) => {
    const { rate } = itemMap.Ink;
    const quantity = 2;
    const stockMovement = await getStockMovement(MovementTypeEnum.MaterialIssue, new Date('2022-11-03T09:59:04.528'), [
        {
            item: itemMap.Ink.name,
...
test('cancel stock movement', async (t) => {
    const names = (await fyo.db.getAllRaw(ModelNameEnum.StockMovement));
    for (const { name } of names) {
        const slesBefore = await getSLEs(name, ModelNameEnum.StockMovement, fyo);
        const doc = (await fyo.doc.getDoc(ModelNameEnum.StockMovement, name));
 ...
 */
async function runEntries(item, entries, t) {
...
//# sourceMappingURL=testInventory.spec.js.map