...
test(' Coupon Codes: create dummy item, party, pricing rules, coupon codes', async (t) => {
    ...
    t.ok(await fyo.db.exists(ModelNameEnum.Party, partyMap.partyOne.name), `Party: ${partyMap.partyOne.name} exists`);
    ...
    for (const pricingRule of Object.values(pricingRuleMap)) {
        ...
        t.ok(await fyo.db.exists(ModelNameEnum.PricingRule, pricingRule.name), `Pricing Rule: ${pricingRule.name} exists`);
    }
    ...
    t.equal(await fyo.db.getStockQuantity(itemMap.Pen.name, locationMap.LocationOne, undefined, undefined), 25, 'Pen has quantity twenty five');
    // Create Coupon Codes
    for (const couponCodes of Object.values(couponCodesMap)) {
        await fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodes).sync();
        t.ok(await fyo.db.exists(ModelNameEnum.CouponCode, couponCodes.name), `Coupoon Code: ${couponCodes.name} exists`);
    }
...
test('Coupon code not created: coupons min amount must be lesser than coupons max.', async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), 'Minimum Amount should be less than the Maximum Amount');
});
test('Coupon code not created: pricing rules max amount is lower than the coupons min.', async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), "Minimum Amount should be greather than the Pricing Rule's Minimum Amount");
});
test('coupon code not created: pricing rules max amount is lower than the coupons max.', async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), "Maximum Amount should be lesser than Pricing Rule's Maximum Amount");
});
test("coupon code is not applied when coupon's validfrom date < coupon's validTo date", async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), 'Valid From Date should be less than Valid To Date');
});
test("coupon code is not applied when coupon's validFrom date < pricing rule's validFrom date", async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), "Valid From Date should be greather than Pricing Rule's Valid From Date");
});
test("coupon code is not applied when coupon's validTo date > pricing rule's validTo date", async (t) => {
    ...
    const ccodeDoc = fyo.doc.getNewDoc(ModelNameEnum.CouponCode, couponCodesMap[0]);
    await assertThrows(async () => await ccodeDoc.sync(), "Valid To Date should be lesser than Pricing Rule's Valid To Date");
});
test('apply coupon code', async (t) => {
    ...
    t.equal(sinv.pricingRuleDetail[0].referenceName, pricingRuleMap[0].name, 'Pricing Rule is applied');
});
...
//# sourceMappingURL=testCouponCodes.spec.js.map