...
test('Pricing Rule: create dummy item, party, pricing rules, free items, locations', async (t) => {
    ...
    for (const pricingRule of Object.values(pricingRuleMap)) {
        ...
        t.ok(await fyo.db.exists(ModelNameEnum.PricingRule, pricingRule.name), `Price List: ${pricingRule.name} exists`);
    }
    // Create Locations
...
test('pricing rule is applied when filtered by min and max qty', async (t) => {
    const pruleDoc = (await fyo.doc.getDoc(ModelNameEnum.PricingRule, pricingRuleMap[0].name));
    ...
    await sinv.runFormulas();
    t.equal(sinv.pricingRuleDetail[0].referenceName, pricingRuleMap[0].name, 'Pricing Rule is added to Pricing Rule Detail');
    t.equal(sinv.items[0].rate.float, pricingRuleMap[0].discountRate, 'item rate fetched from Pricing Rule');
});
...
test('Pricing Rule is not applied when item amount is < min  amount', async (t) => {
    ...
    await sinv.runFormulas();
    t.equal(sinv.pricingRuleDetail?.length, undefined, 'Pricing Rule is not applied');
});
...
test('Pricing Rule is applied when filtered by qty, amount and dates', async (t) => {
    ...
    await sinv.runFormulas();
    t.equal(sinv.pricingRuleDetail[0].referenceName, pricingRuleMap[1].name, 'Pricing Rule is applied');
});
test('Pricing Rule is applied when filtered by qty, amount and dates', async (t) => {
    const sinv = fyo.doc.getNewDoc(ModelNameEnum.SalesInvoice, {
        date: '2024-02-01',
...
test('create two pricing rules, Highest priority pricing rule is applied', async (t) => {
    ...
    await sinv.runFormulas();
    t.equal(sinv.pricingRuleDetail[0].referenceName, 'PRLE-1003', 'Pricing Rule with highest priority is applied');
});
test('Pricing Rule is not applied due to two docs having same priority', async (t) => {
    const pricingRuleDoc = await fyo.doc.getDoc(ModelNameEnum.PricingRule, 'PRLE-1003');
...
test('create a price discount of type percent, discount percent should apply', async (t) => {
    const pricingRuleDoc = await fyo.doc.getDoc(ModelNameEnum.PricingRule, pricingRuleMap[0].name);
...
//# sourceMappingURL=testPricingRule.spec.js.map