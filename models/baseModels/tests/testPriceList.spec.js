...
test('Price List: create dummy item, party, price lists', async (t) => {
    ...
    for (const { name, email } of Object.values(partyMap)) {
        ...
        t.ok(await fyo.db.exists(ModelNameEnum.Party, name), `Party: ${name} exists`);
    }
    for (const priceListItem of Object.values(priceListMap)) {
        await fyo.doc.getNewDoc(ModelNameEnum.PriceList, priceListItem).sync();
        t.ok(await fyo.db.exists(ModelNameEnum.PriceList, priceListItem.name), `Price List: ${priceListItem.name} exists`);
    }
...
test('Check if InvoiceItem rate fetched from PriceList', async (t) => {
    ...
    t.equal(sinv.items?.[0].rate?.float, priceListMap.PL_SELL.priceListItem[0].rate, `sales invoice rate fetched from price list`);
});
closeTestFyo(fyo, __filename);
//# sourceMappingURL=testPriceList.spec.js.map