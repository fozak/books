...
export class InvoiceItem extends Doc {
    constructor(schema, data, fyo) {
        ...
        this.formulas = {
            ...
            itemCode: {
                formula: async () => (await this.fyo.getValue('Item', this.item, 'itemCode')),
                dependsOn: ['item'],
            },
            rate: {
                formula: async (fieldname) => {
                    ...
                    const rateFromTotals = getRate(quantity, itemDiscountPercent, itemDiscountAmount, totalTaxRate, itemTaxedTotal, itemDiscountedTotal, isItemTaxedTotal, discountAfterTax, setItemDiscountAmount);
            ...
            unitConversionFactor: {
                formula: async () => {
                    ...
                    const conversionItems = await this.fyo.db.getAll(ModelNameEnum.UOMConversionItem, {
                        ...
                        filters: { parent: this.item, uom: this.transferUnit },
                    });
                    this.quantity =
                        conversionItems[0]?.conversionFactor *
            ...
            tax: {
                formula: async () => {
                    const itemTax = (await this.fyo.getValue('Item', this.item, 'tax'));
                    if (itemTax) {
                        return itemTax;
                    }
                    const itemGroup = (await this.fyo.getValue('Item', this.item, 'itemGroup'));
                    if (!itemGroup) {
                        return '';
                    }
                    const itemGroupDoc = await this.fyo.doc.getDoc('ItemGroup', itemGroup);
                    return itemGroupDoc?.tax;
                },
                dependsOn: ['item'],
            },
            amount: {
            ...
            itemDiscountedTotal: {
                formula: async () => {
                    ...
                    if (!this.discountAfterTax) {
                        return getDiscountedTotalBeforeTaxation(rate, quantity, itemDiscountAmount, itemDiscountPercent, !!this.setItemDiscountAmount);
                    }
                    return getDiscountedTotalAfterTaxation(totalTaxRate, rate, quantity, itemDiscountAmount, itemDiscountPercent, !!this.setItemDiscountAmount);
                },
                dependsOn: [
                    'itemDiscountAmount',
                    'itemDiscountPercent',
            ...
            itemTaxedTotal: {
                formula: async () => {
                    ...
                    const itemDiscountPercent = this.itemDiscountPercent ?? 0;
                    if (!this.discountAfterTax) {
                        return getTaxedTotalAfterDiscounting(totalTaxRate, rate, quantity, itemDiscountAmount, itemDiscountPercent, !!this.setItemDiscountAmount);
                    }
            ...
            stockNotTransferred: {
                formula: async () => {
                    ...
                    const item = (await this.loadAndGetLink('item'));
                    if (!item.trackItem) {
                        return 0;
                    }
                    ...
                    const refdoc = (await this.fyo.doc.getDoc(stockTransferSchemaName, backReference));
                    const transferred = refdoc.items
            ...
            setItemDiscountAmount: {
                formula: async () => {
                    ...
                    const hasPricingRule = this.parentdoc?.pricingRuleDetail?.some((rule) => rule.referenceItem === this.item);
                    if (!hasPricingRule && this.itemDiscountAmount.isZero()) {
                        return false;
                    }
                    const applicablePricingRules = await getPricingRule(this.parentdoc);
            ...
            itemDiscountPercent: {
                formula: async () => {
                    ...
                    const pricingRule = this.parentdoc?.pricingRuleDetail?.filter((prDetail) => prDetail.referenceItem === this.item);
                    ...
                    const pricingRuleDoc = (await this.fyo.doc.getDoc(ModelNameEnum.PricingRule, pricingRule[0].referenceName));
        ...
        this.validations = {
            rate: (value) => {
                ...
                throw new ValidationError(this.fyo.t `Rate (${this.fyo.format(value, 'Currency')}) cannot be less zero.`);
            },
            itemDiscountAmount: (value) => {
                if (value.lte(this.amount)) {
                    return;
                }
                throw new ValidationError(this.fyo.t `Discount Amount (${this.fyo.format(value, 'Currency')}) cannot be greated than Amount (${this.fyo.format(this.amount, 'Currency')}).`);
            },
            itemDiscountPercent: (value) => {
                if (value < 100) {
                    return;
                }
                throw new ValidationError(this.fyo.t `Discount Percent (${value}) cannot be greater than 100.`);
            },
    ...
    async getTotalTaxRate() {
        ...
        const details = (await this.fyo.getValue('Tax', this.tax, 'details')) ?? [];
    ...
    _setGetCurrencies() {
        ...
        for (const { fieldname } of currencyFields) {
            (_b = this.getCurrencies)[fieldname] ?? (_b[fieldname] = this._getCurrency.bind(this));
        }
    }
}
_a = InvoiceItem;
InvoiceItem.filters = {
    ...
    batch: async (doc) => {
        ...
        const batchName = batches.map((b) => b.name);
        return {
            name: ['in', batchName],
        };
    },
    transferUnit: async (doc) => {
        const conversionItems = await doc.fyo.db.getAll(ModelNameEnum.UOMConversionItem, {
            fields: ['uom'],
            filters: { parent: doc.item },
        });
        const conversionUoms = conversionItems.map((i) => i.uom);
        return {
            name: ['in', conversionUoms],
        };
    },
};
InvoiceItem.createFilters = {
    item: (doc) => {
        return { for: doc.isSales ? 'Sales' : 'Purchases' };
    },
};
async function getItemRate(doc) {
    if (doc.isFreeItem) {
        return doc.rate;
    }
    let pricingRuleRate;
    ...
    if (doc.fyo.singles.AccountingSettings?.enablePriceList) {
        priceListRate = await getItemRateFromPriceList(doc, doc.parentdoc?.priceList);
    }
    if (priceListRate) {
...
async function getItemRateFromPricingRule(doc) {
    const pricingRule = doc.parentdoc?.pricingRuleDetail?.filter((prDetail) => prDetail.referenceItem === doc.item);
    if (!pricingRule || !pricingRule.length) {
        return;
    }
    const pricingRuleDoc = (await doc.fyo.doc.getDoc(ModelNameEnum.PricingRule, pricingRule[0].referenceName));
...
function getDiscountedTotalBeforeTaxation(rate, quantity, itemDiscountAmount, itemDiscountPercent, setDiscountAmount) {
    /**
...
function getTaxedTotalAfterDiscounting(totalTaxRate, rate, quantity, itemDiscountAmount, itemDiscountPercent, setItemDiscountAmount) {
    /**
     * If Discount is applied before taxation
     * Formula: Discounted Total * (1 + TotalTaxRate / 100)
     */
    const discountedTotal = getDiscountedTotalBeforeTaxation(rate, quantity, itemDiscountAmount, itemDiscountPercent, setItemDiscountAmount);
    return discountedTotal.mul(1 + totalTaxRate / 100);
}
function getDiscountedTotalAfterTaxation(totalTaxRate, rate, quantity, itemDiscountAmount, itemDiscountPercent, setItemDiscountAmount) {
    /**
...
function getTaxedTotalBeforeDiscounting(totalTaxRate, rate, quantity) {
    /**
     * If Discount is applied after taxation
...
function getRate(quantity, itemDiscountPercent, itemDiscountAmount, totalTaxRate, itemTaxedTotal, itemDiscountedTotal, isItemTaxedTotal, discountAfterTax, setItemDiscountAmount) {
    ...
    if (isItemDiscountedTotal && discountAfterTax && !setItemDiscountAmount) {
        return itemDiscountedTotal.div((quantity * (100 - itemDiscountPercent) * (100 + totalTaxRate)) / 100);
    }
    ...
    if (isItemTaxedTotal && discountBeforeTax && !setItemDiscountAmount) {
        return itemTaxedTotal.div(quantity * (1 - itemDiscountPercent / 100) * (1 + totalTaxRate / 100));
    }
    return null;
}
