...
export class Party extends Doc {
    ...
    async _getTotalOutstandingAmount(schemaName) {
        ...
        return outstandingAmounts
            .map(({ outstandingAmount }) => this.fyo.pesa(outstandingAmount))
        ...
        return [
            {
                ...
                condition: (doc) => !doc.notInserted && doc.role !== 'Customer',
                action: async (partyDoc, router) => {
                    const doc = fyo.doc.getNewDoc('PurchaseInvoice', {
                        party: partyDoc.name,
                        account: partyDoc.defaultAccount,
                    });
                    await router.push({
            ...
            },
            {
                ...
                condition: (doc) => !doc.notInserted && doc.role !== 'Supplier',
                action: async (partyDoc, router) => {
