...
// prettier-ignore
export const partyPurchaseItemMap = {
...
export const purchaseItemPartyMap = Object.keys(partyPurchaseItemMap).reduce((acc, party) => {
...
export function getFlowConstant(months) {
    // Jan to December
    const d = DateTime.now().minus({ months });
    return flow[d.month - 1];
}
export function getRandomDates(count, months) {
    /**
