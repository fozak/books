...
test('check setup Singles', async (t) => {
    ...
    for (const field of setupFields) {
        ...
        const optionsValue = setupOptions[field];
        if (dbValue instanceof Date) {
            dbValue = DateTime.fromJSDate(dbValue).toISODate();
        }
        t.equal(dbValue, optionsValue, `${field}: (${dbValue}, ${optionsValue})`);
    }
});
test('check null singles', async (t) => {
    ...
    t.equal(nullSingles.length, 0, `null singles: ${JSON.stringify(nullSingles)}`);
});
test.onFinish(async () => {
    await fyo.close();
});
//# sourceMappingURL=testSetupInstance.spec.js.map