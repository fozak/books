...
export class GeneralLedger extends LedgerReport {
    ...
    _convertEntriesToReportData(entries) {
        const reportData = [];
    ...
    _getRowFromEntry(entry, columns) {
        if (entry.name === -3) {
            return {
                isEmpty: true,
                cells: columns.map((c) => ({
    ...
    _getQueryFilters() {
        ...
        if (this.toDate) {
            ...
            filters.date.push('<=', this.toDate);
        }
        if (this.fromDate) {
            filters.date ?? (filters.date = []);
            filters.date.push('>=', this.fromDate);
        }
        if (!this.reverted) {
    ...
    getFilters() {
        ...
        if (!this.fyo.singles.AccountingSettings?.enableInventory) {
            refTypeOptions.push({ label: t `Shipment`, value: 'Shipment' }, { label: t `Purchase Receipt`, value: 'PurchaseReceipt' });
        }
        return [
            {
                fieldtype: 'Select',
...
GeneralLedger.title = t `General Ledger`;
GeneralLedger.reportName = 'general-ledger';
